...
export default async function AiArticle({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = getArticle('ai', slug);
  if (!article) notFound();

  const related = getRelatedArticles('ai', slug, 3);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 pt-24 pb-20">
        <article className="max-w-3xl mx-auto px-6">
          ...
          <div className="mb-3">
            <span className="text-[10px] px-2 py-0.5 rounded-full border border-border/50 text-muted-foreground/60">AI</span>
          </div>
