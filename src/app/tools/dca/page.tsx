...
interface PricePoint { date: string; price: number; normalized: number; }
...
export default function DcaBacktest() {
  ...
  const result = useMemo(() => {
    ...
    const finalPrice = prices[prices.length - 1]?.price || 0;
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 pt-16">
        <div className="max-w-3xl mx-auto px-6 w-full py-20">
          <h1 className="text-3xl font-bold tracking-tight text-foreground mb-2">定投回测</h1>
          <p className="text-muted-foreground mb-12">选择标的和日期范围，模拟每月定投的真实收益。</p>
          ...
          {prices.length > 0 && !loading && (
            <div className="text-xs text-muted-foreground/50 mb-8">
              {startDate} ~ {endDate}，共 {prices.length} 个月数据
            </div>
          )}

          {loading && (
            <div className="rounded-xl border border-border bg-card/30 p-12 text-center mb-8">
              <div className="text-sm text-muted-foreground/60">正在获取历史数据...</div>
            </div>
          )}

          {error && (
            <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-6 text-center mb-8">
              <div className="text-sm text-red-400/80 mb-1">获取失败</div>
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
                <div className="rounded-xl border border-border bg-card/50 p-4">
                  <div className="text-[10px] text-muted-foreground/50 uppercase tracking-wider mb-1">定投期数</div>
                  <div className="text-xl font-bold text-foreground">{result.monthlyData.length}<span className="text-xs text-muted-foreground/50 ml-1">个月</span></div>
                </div>
                <div className="rounded-xl border border-border bg-card/50 p-4">
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    ...
                    {result.monthlyData.length > 8 && (
                      <>
                        <tr className="border-b border-border/50"><td className="px-4 py-2 text-muted-foreground/40 text-center" colSpan={4}>· · ·</td></tr>
                        {(() => {
          ...
          <div className="mt-8 text-xs text-muted-foreground/40 text-center">
            {prices.length > 0 ? <>数据来源: Yahoo Finance · 投资有风险，决策需谨慎</> : <>投资有风险，决策需谨慎</>}
          </div>
        </div>
      </main>
      <Footer />
